...
class SARIMAXModel:
    def __init__(self, freq: str):
        ...
        self.jarquebera_p_thershold = None
    ...
    @staticmethod
    def make_adf_test(endog: pd.Series, autocorr_lags: list|tuple) -> None:
        print('ADF Test')
    ...
    def optimize_reg_params(self, endog_train: pd.Series, endog_test: pd.Series, forecast_length: int,
        ...
        self.jarquebera_p_thershold = jarquebera_p_thershold
        ...
        for params_tuple in itertools.product(p_params, d_params, q_params,
            ...
            try:
                ...
                self.make_insample_prediction(1, len(endog_train) - 1)
    ...
    def make_jarquebera_test(self) -> bool:
        ...
        return jb_test[0][1] < self.jarquebera_p_thershold

    def make_forecast(self, forecast_length: int, exog_test: pd.DataFrame, conf_int_alpha: float = 0.05) -> None:
        forecast = self.res.get_forecast(steps=forecast_length, exog=exog_test)
    ...
    def make_insample_prediction(self, start: int, end: int, conf_int_alpha: float = 0.05):
    ...
    def print_results(self):
        ...
        print(f'Mean insample prediction error: {self.insample_prediction_df['prediction_error'].mean()}')
        print(f'Mean insample prediction error percentage: {
            self.insample_prediction_df["prediction_error_percentage"].mean()}')
        print(f'Mean insample pred. relative confidence interval width: {
        ...
        axes.plot(self.insample_prediction_df['predicted_mean'], color='blue', label='Insample prediction')
